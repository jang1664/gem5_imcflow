...
`ifdef BIG_IMEM
    ...
    $display("[%0t] [SRAM_DIRECT] IMEM WRITE (BIG_IMEM behavioral): inode=%0d, byte_addr=0x%04x (sram[0x%03x]), data=0x%08x",
...
`ifdef BIG_IMEM
    ...
    endcase
    $display("[%0t] [SRAM_DIRECT] IMEM READ (BIG_IMEM behavioral): inode=%0d, byte_addr=0x%04x (sram[0x%03x]), data=0x%08x",
             $time, inode_id, byte_addr, word_addr, data);
`else
    logic [7:0] word_addr;
    logic [4:0] row_addr;    // For compiled memory: 32 rows
...
`endif
    ...
    forever begin
      ...
      if (result > 0) begin
        ...
        if (transaction_received_count == 0) begin
          ...
          // Got transactions, now wait for more before assuming the test is done.
          // Deep models (e.g. VWW MobileNet, 13 blocks) have long host-side gaps
          // between accelerator kernels; a 100k-cycle (~1ms) idle timeout fires
          // mid-run and closes the socket while gem5 is still preparing the next
          // kernel -> Broken pipe panic in imcflow_pio_socket.cc. Use 5M cycles
          // (~50ms). The global 300s watchdog and MaxCycle timeout below still
          // bound a genuinely hung run.
          if (no_transaction_count > 5000000) begin
  ...
  initial begin
    #3_000_000_000_000; // 3 trillion time units (sim-time, not wall-clock).
    // Raised 10x from 300G: deep models (VWW, 13 blocks / 260 graph nodes) run
    // well past the old bound. This is simulated time, so a larger value is
    // harmless; a genuinely hung run is still caught by MaxCycle below.
    $display("[%0t] [SV] GLOBAL TIMEOUT - forcing finish", $time);
    socket_server_close();
    $finish;
  end
